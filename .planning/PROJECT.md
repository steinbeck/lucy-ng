...
---
*Last updated: 2026-07-13 — Phase 98 (Reconstruction + Processing) COMPLETE & verified (VERIFICATION passed, 5/5 must-haves, RECON-01..05 validated; code review 0 critical / 4 warning all fixed; full suite 1329 passed). Shipped `lucy nus reconstruct` — the full headless bruk2pipe→nusExpand→SMILE→FT/phase/baseline chain (FnMODE-branched stage order, F2-before-F1, fail-loud per-stage `run_stage()`, convergence-based SMILE stopping, reversed 1D-calibrated ppm axes); QF/COSY branch + F1 phase defaults PROVISIONAL pending Phase-100 real-data validation. Next: Phase 99 (Peak-Pick Bridge + QC Gate + CLI).*
